...
dates_minus_2 <- lapply(dates, function(d) date(as.POSIXct(d) - days(2)))
dates_minus_7 <- lapply(dates, function(d) date(as.POSIXct(d) - days(7)))
...
query1 <- function(date, date_minus_2, date_minus_7) {
          paste0("SELECT  CASE WHEN future_payers.user_id IS NULL THEN 0 ELSE 1 END AS future_payer,
                          CONVERT(VARCHAR(40), current_transaction.user_id),
...
saveRDS(payer_again_data, file=paste0("./payer_again_dc/production/input_data/training_data",dates[[1]],".rds"))
