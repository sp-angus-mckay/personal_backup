...
### TO DO:
### -change for TV as looking at different periods - maybe do per day etc
...
campaign_type <- 'TV' # 'TV' or 'Youtube'
campaign_start  <- '2016-07-23 12:29:00'
analysis_period <- 30 # in days
...
benchmark_query <- paste( "SELECT date_register, register_ip_timezone, user_id
                         ...
                         AND (register_source is NULL or lower(register_source) like '%organic%' or register_source = '')
                         AND register_platform = '", platform, "'
                         AND register_ip_country ", bm_countries, sep = "")
